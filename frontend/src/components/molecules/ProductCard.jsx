...
const ProductCard = ({ product }) => {
  ...
  const handleAddToCart = (e) => {
    e.preventDefault();
    if (!user) return toast({ title: "Please login to add items to cart", variant: "destructive" });
    if (user.role !== "customer") return toast({ title: "Only customers can add to cart", variant: "destructive" });
    addToCart.mutate(
      ...
      <Card className="group overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1 animate-fade-in">
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <Button size="sm" onClick={handleAddToCart} disabled={addToCart.isPending}>
